...
def signup_page(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request, f"Hi {user} your account has been created successfully. Contact admin for your username and password")
...
def admin_panel(request):
    ...
    if request.user.is_superuser:
        ...
        approved_customer = CustomerDetails.objects.filter(
            registration_status="Approved")
        new_registrations = CustomerDetails.objects.filter(
...
def approve_registration(request, customer_id):
    ...
    if request.method == 'POST':
        ...
        updated_customer = CustomerDetails.objects.filter(
    ...
    if request.user.is_superuser:
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                customer_id = CustomerDetails.objects.get(
                    customer_id=customer_id)
                facility_accessible = CustomerFacility.objects.filter(
                    customer=customer_id).filter(facility=facility)

                if facility_accessible:
                    form.save()
                    CustomerReservations.objects.update(
                        customer_id=customer_id)
                    messages.success(request, "Booking confirmed!")

                else:
                    messages.error(
                        request, "Your are not eligible to book this facility")
                return redirect('sub')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if payment_made >= 50 and payment_made < 100:
                update_customer_facility(
                    customer_id=customer_id, facility_name='Swimming Pool')

            elif payment_made >= 100:
                update_customer_facility(
                    customer_id=customer_id, facility_name='Tennis Court')
                update_customer_facility(
                    customer_id=customer_id, facility_name='Conference Room')
                update_customer_facility(
                    customer_id=customer_id, facility_name='Swimming Pool')
